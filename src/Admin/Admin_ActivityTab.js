...
const Admin_ActivityTab = () => {
        ...
        const fetchIncluded = async () => {
            try {
                const response = await axios.post(
                    `${config.API_BASE_URL}/api/admin/getall_activitytab`
                );
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this trip Activity Tab?")) {
          try {
            const response = await axios.delete(
              `${config.API_BASE_URL}/api/admin/delete_activitytab/${id}`
            );
        ...
        <div>
            <div className="container-fluid">
                <div className="row m-2">
                    <div className="col-6">
                        <h1>All Trip Activity Tab</h1>
                    </div>
                    <div className="col-6">
                        <Link to="/admin/add/activitytab" className="btn btn-success float-end">
                            Add New Activity Tab
                        </Link>
                    </div>
                </div>
                        ...
                        <tbody>
                            {tripIncluded.map((trip) => (
                                <tr key={trip._id}>
                                    <td>{trip.tripName}</td>
                                    <td>
                                        {/* <Link className="btn btn-primary btn-sm me-2" to={`/admin/edit/activitytab/${trip._id}`}>Edit</Link> */}
